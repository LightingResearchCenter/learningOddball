function epochs_ep = denoise_ep_den_auto_Wrapper(epochs, scales, parameters, handles)
    ...
    epochs_ep.RT_regular = epochs.RT_regular;
    epochs_ep.RT_irregular = epochs.RT_irregular;
    
    % also the samples per epoch
    epochs_ep.samplesPerEpoch = epochs.samplesPerEpoch;
    
    % same for indices
    epochs_ep.irregularIndices = epochs.irregularIndices;
    epochs_ep.regularIndices = epochs.regularIndices;
    ...
    % other words, as the epochs are concatenated into single vector
        ...
        [rowsIn, colsIn] = size(epochs.oddball_irregular);
        
        % Irregulars
        for i = 1 : parameters.EEG.nrOfChannels
            epochs_ep.oddball_irregular(:,i) = epLoopFunction(i, epochs.oddball_irregular(:,i), parameters, scales, den_type, epochs.samplesPerEpoch, handles);
        end

        % Regulars
        for i = 1 : parameters.EEG.nrOfChannels
            epochs_ep.oddball_regular(:,i) = epLoopFunction(i, epochs.oddball_regular(:,i), parameters, scales, den_type, epochs.samplesPerEpoch, handles);
        end
