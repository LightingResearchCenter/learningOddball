...
noOfBlockLoops = 6 # default is 6
noOfIregularTargetLoops = 8 # default is 8
noOfRegularTargetLoops = 8 # default is 8
...
for thisCycleLoop in CycleLoop:
    ...
    irregularTargets = data.TrialHandler(nReps=noOfIregularTargetLoops, method=u'sequential', # NOTE! Randomized after this "manually" with constraints
        extraInfo=expInfo, originPath=None,
        trialList=data.importConditions('Oddballs_irregular_WavFiles.csv'),
        ...
        while continueRoutine:
            ...
            if usePyo == 1:
                ...
                if frameN >= 0 and soundStimulus.status == NOT_STARTED:
                    ...
                    soundStimulus.frameNStart = frameN  # exact frame index
                    soundStimulus.play()  # start the sound (it finishes automatically)
                #elif soundStimulus.status == STARTED and  t >= triggerDuration:
                #    d.getFeedback(u6.BitStateWrite( 3, 0 ), u6.BitStateWrite( 2, 0 ) ) # std and odd LOW
    ...
    print " "
    print "NEW LOOP (Regular)"         
        # .TrialHandler called upon every new loop, causing a delay of ~75 ms in the current system

    # LOOP for REGULAR TARGETS
    regularTargets = data.TrialHandler(nReps=noOfRegularTargetLoops, method=u'sequential', 
        extraInfo=expInfo, originPath=None,
        trialList=data.importConditions(u'Oddballs_regular_WavFiles.csv'),
        seed=None, name='regularTargets')

    irregularTargets.data.addDataType('ISI')#this will help store ISI with the stimuli, taken from demo/TrialHandler.py
    d.getFeedback(u6.BitStateWrite( 1, 0 )) # Write irregular boolean is LOW for FO01
    ...
    for thisRegularTarget in regularTargets:
        ...
        Sounds = os.path.join(Subfolder, Filename) # so that relative path works both in Linux/Mac and Win
        # https://groups.google.com/forum/?fromgroups#!topic/psychopy-users/twtPKicPGeQ
        # LearningOddballParadigm.setSound(Sounds) # for pygame (not for pyo)
        # print Sounds
        # print thisIrregularTarget
        # soundStimulus.setSound(Sounds)
        ...
        if oddBallYes== 1:
            ...
            ISI_ms = 1000*(timeNow-timePrev).total_seconds()
            print "      Regular oddball, ISI:", ISI_ms, " ms", ", index: ", regularTargets.thisIndex+1, "/", tonesPerBlock, ", N: ", regularTargets.thisN, "/", regularTargets.nTotal
            irregularTargets.data.add('ISI', ISI_ms)
            ...
            ISI_ms = 1000*(timeNow-timePrev).total_seconds()
            print "   Regular standard, ISI:", ISI_ms, " ms"
            irregularTargets.data.add('ISI', ISI_ms)
        ...
        while continueRoutine:
            ...
            if usePyo == 1:
                ...
                # elif  soundStimulus.isPlaying == True and t >= triggerDuration:
                #   d.getFeedback(u6.BitStateWrite( 3, 0 ), u6.BitStateWrite( 2, 0 ) ) # std and odd LOW
                elif soundStimulus.isPlaying == True and t >= (soundStimulus.tStart + soundDuration):                
                    soundStimulus.stop()  # stop the sound (if longer than duration)            
            else:
                if frameN >= 0 and soundStimulus.status == NOT_STARTED:
                    # keep track of start time/frame for later
                    ...
                    soundStimulus.frameNStart = frameN  # exact frame index
                    soundStimulus.play()  # start the sound (it finishes automatically)
                elif soundStimulus.status == STARTED and triggerOff == 0 and t >= (soundStimulus.tStart + soundDuration):                
                    soundStimulus.stop()  # stop the sound (if longer than duration)            
                    triggerOff = 1; # reset flag, only goes once inside this
                elif soundStimulus.status == STARTED and t >= (soundStimulus.tStart + soundDuration):
                    soundStimulus.stop()  # stop the sound (if longer than duration)
...
d.getFeedback(u6.BitStateWrite( 1, 0 ) ) # FI01 (1), set LOW (0), i.e. IRREGULAR Boolean YES
...
theorDuration = noOfBlockLoops * ((noOfIregularTargetLoops*8)+(noOfRegularTargetLoops*8))*0.8
